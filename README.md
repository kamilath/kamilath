<div align="center">

# 👩🏻‍💻 KAMILATH RIFKA

### `SDET-1` · `Automation Engineer` · `AI Enthusiast`

**Building reliable automation. Exploring AI. Solving problems.**

<br>

<a href="https://www.linkedin.com/in/kamilath-rifka/">
<img src="https://img.shields.io/badge/💼%20LINKEDIN-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:kamilathrifka@gmail.com">
<img src="https://img.shields.io/badge/✉%20EMAIL-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</div>

---

<div align="center">

```text
╭──────────────────────────────────────────────────────────╮
│                                                          │
│   $ whoami                                               │
│                                                          │
│   Kamilath Rifka                                         │
│   SDET-1                                                 │
│                                                          │
│   $ focus                                                │
│                                                          │
│   Playwright  •  Selenium  •  API Testing                │
│   TypeScript  •  Java  •  Python                         │
│   AI Engineering  •  CI/CD                               │
│                                                          │
╰──────────────────────────────────────────────────────────╯
```

</div>

## 👋 About Me

Hi! I'm **Kamilath Rifka S**, an **SDET-1** who enjoys turning repetitive testing into reliable automation.

My work revolves around **UI automation, API validation, scalable test frameworks, accessibility testing, and CI/CD**. Outside testing, I enjoy building **AI-powered applications and developer tools**.

```text
Test → Automate → Analyze → Improve
```

---

## 🧪 Automation Lab

### 🩹 [Self-Healing Playwright](https://github.com/kamilath/self-healing-playwright)

**TypeScript · Playwright · Page Object Model**

A Playwright framework built to handle **dynamic and changed UI locators** through self-healing automation.

`50+` automated tests · `20+` self-healing scenarios · `15+` reusable POM components

**→ [View Repository](https://github.com/kamilath/self-healing-playwright)**

---

### 🤖 [AI Interview Assistant](https://github.com/kamilath/AI-Interview-Assistant)

**React · TypeScript · Ollama · REST APIs**

An AI-powered interview platform connecting **resume analysis, job requirements, skill matching, and personalized interview preparation**.

`500+` analyses · `85%+` matching accuracy · `5+` REST APIs · `1,000+` questions

**→ [View Repository](https://github.com/kamilath/AI-Interview-Assistant)**

---

## 🚀 More Things I've Built

<table>
<tr>
<td width="50%">

### 🛍️ E-Commerce Web App

A bilingual **Tamil + English** shopping platform with Firebase authentication, dark mode and an AI chatbot.

**React · JavaScript · Firebase**

🤖 50+ product & FAQ queries

[Live Demo →](https://shopping-cart-rifka.vercel.app)
[Source Code →](https://github.com/kamilath/Shopping-cart)

</td>

<td width="50%">

### 🧾 AI Invoice Processor

AI-assisted invoice validation and categorization system.

**Python · Flask · Gemini API · SQLite**

⚡ Automated invoice processing
📊 Intelligent categorization

[Source Code →](https://github.com/kamilath/Bill-Mate-Effortless-Invoice-Management-with-Intelligent-AI)

</td>
</tr>

<tr>
<td width="50%">

### 🚁 Smart Drone System

Gesture-controlled drone with QR authentication and wireless charging.

**Python · OpenCV · MediaPipe · Arduino**

🎯 92% gesture recognition accuracy

[Source Code →](https://github.com/kamilath/Hand-Gesture-Drone-And-Wireless-Charging-Station)

</td>

<td width="50%">

### 💡 What I Like Building

```text
🧪 Automation frameworks
🤖 AI-powered tools
🌐 Web applications
🔌 REST APIs
⚙️ Developer utilities
```

</td>
</tr>
</table>

---

## 🛠️ Tech Stack

### Languages

<p>
<img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white"/>
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/SQL-003B57?style=flat-square"/>
</p>

### Frontend

<p> <img src="https://img.shields.io/badge/React.js-61DAFB?style=flat-square&logo=react&logoColor=black"/> <img src="https://img.shields.io/badge/Angular.js-DD0031?style=flat-square&logo=angular&logoColor=white"/> <img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white"/> <img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white"/> </p>

### Backend & Databases

<p> <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white"/> <img src="https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white"/> <img src="https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white"/> <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white"/> <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white"/> <img src="https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white"/> <img src="https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white"/> </p>

### Test Automation & Quality

<p>
<img src="https://img.shields.io/badge/Playwright-2EAD33?style=flat-square&logo=playwright&logoColor=white"/>
<img src="https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=selenium&logoColor=white"/>
<img src="https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white"/>
<img src="https://img.shields.io/badge/Swagger-85EA2D?style=flat-square&logo=swagger&logoColor=black"/>
<img src="https://img.shields.io/badge/Axe_Core-111111?style=flat-square"/>
</p>

### Development & AI

<p>
<img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black"/>
<img src="https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white"/>
<img src="https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black"/>
<img src="https://img.shields.io/badge/Ollama-111111?style=flat-square"/>
<img src="https://img.shields.io/badge/Gemini_API-4285F4?style=flat-square&logo=google&logoColor=white"/>
</p>

### DevOps & Tools

<p>
<img src="https://img.shields.io/badge/Jenkins-D24939?style=flat-square&logo=jenkins&logoColor=white"/>
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white"/>
<img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white"/>
<img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/Jira-0052CC?style=flat-square&logo=jira&logoColor=white"/>
</p>

---
## 🧠 Problem Solving

<div align="center">

<a href="https://leetcode.com/u/kamilathrifka/">
<img src="https://leetcard.jacoblin.cool/kamilathrifka?theme=dark&font=Karma"/>
</a>

<br><br>

<a href="https://www.geeksforgeeks.org/profile/kamilatqjkr">
<img src="https://img.shields.io/badge/GeeksforGeeks-View%20Profile-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white"/>
</a>

<br><br>

<b>LeetCode</b> · DSA · Java · Problem Solving
<br>
<b>GeeksforGeeks</b> · DSA · Coding Practice

</div>

---

## 🔭 Currently Exploring

```text
┌────────────────────────────────────────────┐
│                                            │
│  🧪 Advanced Playwright Automation         │
│  🩻 AI-powered QA                         │
│  🔌 API Automation                        │
│  ☕ Java + Spring Boot                    │
│  🐳 Docker & CI/CD                        │
│  🧩 Selenium Framework Design              │
│  🧠 Data Structures & Algorithms           │
│                                            │
└────────────────────────────────────────────┘
```

---

## 🎓 Education

**Bachelor of Technology — Information Technology**

**Sathyabama Institute of Science and Technology, Chennai**

`2021 — 2025` · **CGPA 9.56 / 10**

---


<br>

<div align="center">

**`Automate. Validate. Build. Improve.`**

⭐ Thanks for stopping by!

</div>
